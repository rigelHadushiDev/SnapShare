...
export class FeedService {
    ...
    async getPostsFeed(postsByPage: number = 10, page: number = 1, postCommentsLimit: number = 3, timeDirection = true) {


        let resp = new GetFeedResp();

        let offset = (page - 1) * postsByPage;
        let feedDataTOAlgorithm = await this.getDataToFeedAlgorith(timeDirection, postsByPage, offset);
        // if (timeDirection

        // posts = await this.getPostsByPriority();
    ...
    async getDataToFeedAlgorith(timeDirection: boolean, postsByPage: number, offset = 0) {

        let resp: any;
        let standartLimit: number = 400;
        let feedPostsQuery: string;
        if (timeDirection) {
            ...
            WHERE po.archive = FALSE AND n."networkId" IS NOT NULL AND po."createdAt" >= '${lastSeenTimestamp}'
            GROUP BY 
                po."postId",
                u."profileImg",
                u.username,
            ...
            ORDER BY po."createdAt" DESC
               LIMIT ${standartLimit};`
                ...
                let queryResult = await this.entityManager.query(feedPostsQuery);
                resp = await this.feedAlgorithm(queryResult, queryResult?.length)

                if (queryResult?.length < postsByPage) {

                    let newLimit = postsByPage - queryResult?.length;
                ...
                ORDER BY po."createdAt" DESC
                   LIMIT ${standartLimit};`
                    ...
                    queryResult = await this.entityManager.query(feedPostsQuery);

                    let oldPostsOrder = await this.feedAlgorithm(queryResult, postsByPage);
        ...
        ORDER BY po."createdAt" DESC
           LIMIT ${standartLimit};`;
            ...
            let oldPostsOrder = await this.feedAlgorithm(queryResult, postsByPage, offset) // limit, offset
            resp.push(oldPostsOrder);
        }
